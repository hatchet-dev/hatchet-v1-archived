...
type PATRevokeHandler struct {
	handlers.HatchetHandlerReadWriter
}

func NewPATRevokeHandler(
	config *server.Config,
	decoderValidator handlerutils.RequestDecoderValidator,
	writer handlerutils.ResultWriter,
) http.Handler {
	return &PATRevokeHandler{
		HatchetHandlerReadWriter: handlers.NewDefaultHatchetHandler(config, decoderValidator, writer),
	}
}

func (u *PATRevokeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	pat, err = u.Repo().PersonalAccessToken().UpdatePersonalAccessToken(pat)
