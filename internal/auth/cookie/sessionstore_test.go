...
func newSessionStore(t *testing.T, conf *database.Config, cookieName string) *cookie.UserSessionStore {
	...
	ss, err := cookie.NewUserSessionStore(&cookie.UserSessionStoreOpts{
		SessionRepository:   conf.Repository.UserSession(),
		CookieSecrets:       []string{hashKey, blockKey},
		CookieDomain:        "hatchet.run",
